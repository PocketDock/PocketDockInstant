...
# Uncomment the lines to allow internet access from the PM server
...
iptables -I INPUT -p udp -s 172.19.0.1 -j ACCEPT
iptables -I OUTPUT -p udp -d 172.19.0.1 -j ACCEPT

iptables -A INPUT -p tcp -s 172.19.0.1 -j ACCEPT
iptables -A OUTPUT -p tcp -d 172.19.0.1 -j ACCEPT
